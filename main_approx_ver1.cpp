		...
		for (int i = 0; i < deg_a; i++){
			...
			for (int j = 0; j < deg_b; j++){
				...
				if (threshold_type == "none") {
					...
					if (C_ca) h_motif[h_motif_index] += (int)(hyperedge_b < hyperedge_c);
					else h_motif[h_motif_index]++;
				}
				else {
					th_motif_index = get_motif_index_ab1(size_a, size_b, size_c, C_ab, C_bc, C_ca, g_abc);
					if (C_ca) th_motif[th_motif_index.first][th_motif_index.second] += (int)(hyperedge_b < hyperedge_c);
					else th_motif[th_motif_index.first][th_motif_index.second]++;
				}
			}
	...
	if (threshold_type == "none") {
		...
		for (int i = 1; i < 27; i++){
			for (int j = 0; j < th_motif_kind[i]; j++) {
				...
				th_motif_final[i][j] *= (long double)E / sampling_size;
				th_motif_final[i][j] /= 3;
